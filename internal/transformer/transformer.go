...
// ReasoningTransformer handles reasoning_content for models like DeepSeek
func ReasoningTransformer(req map[string]any, _ string) map[string]any {
	// Convert reasoning to thinking format
...
func TooluseTransformer(req map[string]any, _ string) map[string]any {
	...
	if messages, ok := req["messages"].([]any); ok {
		req["messages"] = append(messages, map[string]any{
			"role":    "system",
			"content": systemReminder,
		})
	}

	// Add ExitTool if tools exist
	if tools, ok := req["tools"].([]any); ok && len(tools) > 0 {
		req["tool_choice"] = "required"
		req["tools"] = append(tools, map[string]any{
			"type": "function",
...
// RewriteSystemPromptTransformer rewrites system prompt from file
func RewriteSystemPromptTransformer(req map[string]any, rewritePath string) map[string]any {
	if rewritePath == "" {
		return req
	}

	// Read the rewrite prompt file
	data, err := os.ReadFile(rewritePath)
	if err != nil {
		return req
	}
	rewritePrompt := string(data)

	// Handle system as array
	if system, ok := req["system"].([]any); ok && len(system) > 1 {
		for i, item := range system {
			if itemMap, ok := item.(map[string]any); ok {
				if text, ok := itemMap["text"].(string); ok && strings.Contains(text, "<env>") {
					// Rewrite the system prompt while preserving <env> section
					parts := strings.SplitN(text, "<env>", 2)
					if len(parts) == 2 {
						itemMap["text"] = rewritePrompt + "<env>" + parts[1]
						system[i] = itemMap
					}
				}
			}
