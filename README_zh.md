...
⚠️ **重要提示**：永远不要将包含真实 API Key 的配置文件提交到 Git！

- `~/.ccg/config.json` 已被 `.gitignore` 自动忽略
- 使用环境变量（如 `${OPENAI_API_KEY}`）代替硬编码的密钥
- `config.example.json` 文件提供了安全的模板（使用占位符）
- 查看 `.gitignore` 了解完整的安全文件忽略列表
